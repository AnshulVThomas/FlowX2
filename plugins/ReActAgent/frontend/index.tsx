...
const ReActAgentUI = ({ id, data, selected }: NodeProps) => {
    ...
    let borderClass = selected ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-200';
    let shadowClass = selected ? 'shadow-[0_0_30px_-5px_rgba(59,130,246,0.6)]' : 'shadow-xl shadow-black/20';
    ...
    return (
        ...
        <div className="relative group rounded-2xl w-[340px] transition-all duration-300">
            ...
            {isRunning && (
                <div className="absolute -inset-[3px] rounded-2xl bg-gradient-to-r from-purple-500 via-indigo-500 to-purple-500 opacity-75 blur-[4px] animate-pulse transition-all duration-1000 group-hover:duration-200" />
            )}
            ...
            {/* INNER CONTENT */}
            <div className={`
                relative flex flex-col w-full h-full rounded-2xl transition-all duration-300
                bg-white border ${borderClass} ${shadowClass}
            `}>
                {/* Header */}
                <div className="relative flex items-center gap-4 p-4 border-b border-gray-100">
                    ...
                    <div className="flex flex-col gap-0.5">
                        <div className="flex items-center gap-2">
                            ...
                            <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-full border ${isRunning ? 'bg-purple-50 border-purple-100' : 'bg-blue-50 border-blue-100'}`}>
                                <Sparkles size={8} className={isRunning ? 'text-purple-500' : 'text-blue-500'} />
                                <span className={`text-[9px] font-bold uppercase tracking-wider ${isRunning ? 'text-purple-600' : 'text-blue-600'}`}>
